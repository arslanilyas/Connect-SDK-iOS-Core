...
- (id)initWithString:(NSString *)string originalURL:(NSURL *)originalURL
    ...
    if (self = [super init]) {
        if ([string isMasterPlaylist]) {
            ...
            self.currentXStreamInf = self.masterPlaylist.xStreamList.lastXStreamInf;
            for (int i=0; i<self.masterPlaylist.xStreamList.count; i++) {
                M3U8ExtXStreamInf *streamInfo = [self.masterPlaylist.xStreamList xStreamInfAtIndex:i];
                if (streamInfo.resolution.height == 1080) {
                    self.currentXStreamInf = streamInfo;
                }
                else if (streamInfo.resolution.height == 1080) {
                    self.currentXStreamInf = streamInfo;
                }
                else if (streamInfo.resolution.height == 720) {
                    self.currentXStreamInf = streamInfo;
                }
                else if (streamInfo.resolution.height == 380) {
                    self.currentXStreamInf = streamInfo;
                }
                else if (streamInfo.resolution.height == 240) {
                    self.currentXStreamInf = streamInfo;
                }
                else if (streamInfo.resolution.height == 240) {
                    self.currentXStreamInf = streamInfo;

...
- (NSInteger)bandwidth {
    if (self.dictionary[M3U8_EXT_X_MEDIA_BANDWIDTH] != nil) {
        return [self.dictionary[M3U8_EXT_X_MEDIA_BANDWIDTH] integerValue];
    }
    else {
        return [self.dictionary[@" BANDWIDTH"] integerValue];
    }
...
- (NSURL *)progressiveURI {
    return [NSURL URLWithString:self.dictionary[M3U8_EXT_X_STREAM_INF_PROGRESSIVE_URI]];
}
